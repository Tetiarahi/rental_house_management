...
<div class="container-fluid">
    <form action="" id="manage-payment">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="ref_number" value="<?php echo isset($ref_number) ? $ref_number : '' ?>" placeholder="Optional reference number">
            <small class="text-muted">Optional - Enter check number, transaction ID, or other reference</small>
        </div>
        <div class="form-group">
            <label for="" class="control-label">Payment Date: </label>
            <input type="date" class="form-control" name="payment_date" value="<?php echo isset($date_created) ? date('Y-m-d', strtotime($date_created)) : date('Y-m-d') ?>" required>
...
<script>
   ...
   $('#tenant_id').change(function(){
    ...
    $('#manage-payment').submit(function(e){
        ...
        var paymentDateStr = $('input[name="payment_date"]').val();
        var todayStr = new Date().toISOString().split('T')[0]; // Get today's date in YYYY-MM-DD format

        console.log('Payment date:', paymentDateStr, 'Today:', todayStr);

        if (paymentDateStr > todayStr) {
            alert_toast("Payment date cannot be in the future.",'error')
            return false;
        }
        ...
        $.ajax({
            ...
            success:function(resp){
                if(resp==1){
                    ...
                    alert_toast("Invalid or inactive tenant selected.",'error')
                    end_load()
                } else {
                    alert_toast("Please check all required fields and try again.",'error')
                    end_load()
                }
            }
        })
